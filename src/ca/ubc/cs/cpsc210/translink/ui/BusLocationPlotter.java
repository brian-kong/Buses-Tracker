...
public class BusLocationPlotter extends MapViewOverlay {
    ...
    public void plotBuses() {
        ...
        busLocationsOverlay.removeAllItems(); // clear it WHOOOoo

        this.busLocationsOverlay = createBusLocnOverlay();
        if (selectedStop != null) {
            for (Bus bus : selectedStop.getBuses()) {
                OverlayItem overlayItem = new OverlayItem(bus.getRoute().getName(), bus.getDestination(),
                        new GeoPoint(bus.getLatLon().getLatitude(), bus.getLatLon().getLongitude()));
